...
public final class SecurityAltGuard {
    ...
    public String onJoin(Player player) {
        ...
        if (total >= IP_LOCK_LIMIT) {
            lockIp(rawIp, total, main, group);
            return "§cTerlalu banyak akun dari koneksi yang sama. IP dikunci. Konfirmasi ke Owner jika ini kesalahan.";
        }
        if (total >= ACCOUNT_BLOCK_LIMIT && main != null && !main.uuid.equals(player.getUniqueId()) && !trusted.contains(player.getUniqueId())) {
            blockAccount(player, total, main, group);
            return "§cAkun ini terdeteksi sebagai akun tambahan dari koneksi yang sama. Hubungi Owner untuk review.";
        }
        return null;
    }
    ...
    public boolean onPayCommand(Player player, String commandLine) {
        ...
        if (sameIpTarget || frozenGroup) {
            ...
            save();
            if (group.size() >= ACCOUNT_BLOCK_LIMIT && main != null && !main.uuid.equals(player.getUniqueId())) {
                blockAccount(player, group.size(), main, group);
            }
    ...
    public void sendHelp(CommandSender sender) {
        ...
        sender.sendMessage(color("&7Rule: &f1 akun normal, 2 akun alert, 3 akun block, 4 keras, 5 IP lock."));
        sender.sendMessage(color("&8&m--------------------------------"));
    }
    ...
    private String actionFor(int total) {
        if (total >= IP_LOCK_LIMIT) return "IP Locked + Owner Review Required";
        if (total >= HARD_ALERT_LIMIT) return "Account Blocked + Hard Alert";
        if (total >= ACCOUNT_BLOCK_LIMIT) return "Account Blocked + Economy Frozen";
