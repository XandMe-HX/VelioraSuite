...
public final class BossConfigManager {
    ...
    private void loadBosses() {
        ...
        for (String id : section.getKeys(false)) {
            ...
            if (!isAllowedBossType(type)) { plugin.getLogger().warning("VelioraBoss: entity boss tidak aman/terbang, skip: " + type); continue; }
            ...
            bosses.put(id.toLowerCase(Locale.ROOT), new BossDefinition(id.toLowerCase(Locale.ROOT), type, boss.getString("display-name", id), rarity, clamp(boss.getDouble("health", 100.0D), 100.0D, 500.0D), clamp(boss.getDouble("damage", 4.0D), 4.0D, 10.0D), Math.max(1.0D, boss.getDouble("scale", 3.0D)), skills.isEmpty() ? defaultSkills() : skills));
        }
    }
    ...
    private boolean isAllowedBossType(EntityType type) { return switch (type) { case WARDEN, RAVAGER, IRON_GOLEM, ZOMBIE, HUSK, DROWNED, PIGLIN_BRUTE, WITHER_SKELETON, VINDICATOR, EVOKER -> true; default -> false; }; }
    private double defaultChance(BossRarity rarity) { return switch (rarity) { case COMMON -> 50.0D; case RARE -> 25.0D; case EPIC -> 15.0D; case LEGENDARY -> 8.0D; case MYTHIC -> 2.0D; }; }
