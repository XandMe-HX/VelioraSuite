...
final class ActionHouseStore {
    ...
    ActionHouseStore(VelioraSuite plugin) {
        this.plugin = plugin;
    ...
    void load() {
        ...
        if (profiles != null) for (String raw : profiles.getKeys(false)) try {
            ...
            slots.put(uuid, Math.max(1, yaml.getInt(p + "slots", 8)));
    ...
    void save() {
        ...
        for (UUID owner : owners) { String p = "profiles." + owner + "."; yaml.set(p + "pending", pending.getOrDefault(owner, 0D)); yaml.set(p + "slots", slots.getOrDefault(owner, 8)); yaml.set(p + "expired", expired.getOrDefault(owner, List.of())); }
    ...
    int capacity(UUID owner) { return slots.getOrDefault(owner, 8); }
